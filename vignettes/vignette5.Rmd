...
buildSeed <- function(trainSet, genesInSeed=200, groupSize=30, randomize=TRUE, num.trees=1000, plotIt=TRUE, trainSet.3sam=NULL, trainSet.30sam=NULL, proportional=FALSE) {
  ...
  if(plotIt==TRUE) { pheatmap(seedMat, main='Marker Gene Matrix, train, log2(x+1)') }
  return(seedMat)
}
```
...
```{r,echo=FALSE}
#' Generate all the signature matrices in one call
#' @description  This wrapper is helpful for repetitively matrix generation. It generates seed matrix, all-gene matrix, augmented matrix, shrunk matrix, and all the clustered matrices in one call.
#'
...
#' @param proportional Set to true to make the training set cell type proportional.  Ignores group size (DEFAULT: FALSE)
#' @param handMetaCluster Load in pre-defined meta clusters. Set to NULL to automatically group indistinguishable #' cells into same clusters use clustWspillOver (DEFAULT: NULL)
#'
#' @export
...
scMatrixTest<-function(exprData, randomize = TRUE, skipShrink=FALSE, proportional=FALSE,handMetaCluster=NULL) {
  ...
  trainTestSet <- ADAPTS::splitSCdata(exprData, numSets=2, randomize =randomize ,verbose = FALSE)
  ...
  #shrink
  if(skipShrink == FALSE) {
    augTrain.shrink <- ADAPTS::shrinkSigMatrix(sigMatrix=augTrain, numChunks=NULL, verbose=FALSE, plotIt=TRUE, aggressiveMin=TRUE,sigGenesList=NULL, fastStop=FALSE, singleCore=TRUE)
...
pheatmap(augTrain.shrink, main=titleStr)
  ...
  names(metaClust.LUT.MI) <- names(metaClust.LUT)
    ...
    #Metaseed
    genesInSeed<-100
    metaseedMat <-buildSeed(metatrainSet, genesInSeed=genesInSeed, groupSize=30, randomize=TRUE, num.trees=1000, plotIt=TRUE, trainSet.3sam=metatrainSet.3sam, trainSet.30sam=metatrainSet.30sam,proportional = proportional)
    ...
    if(skipShrink == FALSE) {
      ...
      metatitleStr <- paste('Shrunk Signature Matrix,', '# Genes:',nrow(meta.augTrain),'->',nrow(meta.augTrain.shrink))
pheatmap(meta.augTrain.shrink, main=metatitleStr)
...
```{r convergenceFunction, echo=FALSE}
#' Find convergence iteration
#' @description Find out at which iteration the results converge, i.e. the mean results are stable
#'
...
#' @description Calculate the mean and the standard deviation of the reaults from all the iterations, and also 
...
#' @param changePer The maximum percentage of change allowed for convergence#' 
#' @param handMetaCluster Load in pre-defined meta clusters. Set to NULL to automatically group indistinguishable #' cells into same cluster use clustWspillOver(DEFAULT: NULL)
...
```{r}
print(resList$estimates.onTest)
print(resList$estimates.onTest.meta)

```

multirun
```{r,fig.show = 'hide',results='hide'}
allResList<-loopTillConvergence(numLoops=3,fastStop=FALSE,exprData=toy,changePer = 50,handMetaCluster = NULL)
meanRes<-meanResults(allResList,changePer = 50)
```
